...
function get_ten_transactions(callback){
     connection.query("select Username, name from Borrows, Item, User where idItem = idProduct and Borrows.idUser=User.idUser order by Borrows.inital_date desc limit 10",function(err1,transactions) {
        connection.query("select Username from Borrows, Item, User where idItem = idProduct and Item.owner=User.idUser order by Borrows.inital_date desc limit 10",function(err2,lenders) {        
            if(err1){
...
function updateFeed(idBorrows, callback){
        connection.query("select Username, name from Borrows, Item, User where idBorrows = ? and Borrows.idUser=User.idUser order by Borrows.inital_date desc limit 10", [idBorrows], function(err1,transactions) {
        connection.query("select Username from Borrows, Item, User where idBorrows = ? and Item.owner=User.idUser order by Borrows.inital_date desc limit 10", [idBorrows],function(err2,lenders) {        
            if(err1){
                console.log('err1'+err1);
            }else if(err2){
                console.log('err2'+err2);
            }else{
                io.sockets.emit("new_transaction", transactions, lenders);
            }
     });
    });
};
